...
exports.updateAdminPhone = async(request,response)=>{
    let sql="UPDATE Admin SET Admin_Phone=?,Admin_City=?,Admin_Name=? WHERE Admin_Email=?"
    let result1=await db.connection.execute(sql,[request.body.Admin_Phone,request.body.Admin_City,request.body.Admin_Name,request.body.Admin_Email]);
