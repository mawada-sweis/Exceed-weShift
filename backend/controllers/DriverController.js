...
exports.updateDriverPhone = async(request,response)=>{
    let sql="UPDATE Driver SET Driver_Phone=?,Driver_Req_Type=?,Driver_Shifts_Number=?,Driver_City=? WHERE Driver_Email_PK=?"
    let result1=await db.connection.execute(sql,[request.body.Driver_Phone,request.body.Driver_Shifts_Number, request.body.Driver_Req_Type, request.body.Driver_City, request.body.Driver_Email_PK]);
