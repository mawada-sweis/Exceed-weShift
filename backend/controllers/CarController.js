...
exports.getAllCar = async (request, response) => {
    let [result, rows] = await db.query("SELECT * FROM Car")
    console.log("shgsjad");
    response.status(200).json(result)
}


exports.getAllCar = (request, response) => {
db.query("SELECT * FROM Car", function (err, result, fields) {
    if (err) throw err;
    return(result);
  });}

  // post (add) new car  
exports.postAllCar = (request, response) => {
      db.execute("insert into Car (Car_ID_PK ,Car_Model,Car_Number,Expenses,Insurance_License,Car_Fuel_Type,Car_Fuel_Price,Car_Type,Car_Year_Release,Car_Passenger_Number ) values (?,?,?,?,?,?,?,?,?,?)",
       [request.body.Car_ID_PK,request.body.Car_Model,request.body.Car_Number,request.body.Expenses,request.body.Insurance_License,request.body.Car_Fuel_Type,request.body.Car_Fuel_Price,request.body.Car_Type,request.body.Car_Year_Release,request.body.Car_Passenger_Number] , function (err, result, fields) {
        if (err) throw err;
        return(result);
      });}
  */
